    ...
    >
      {value.map((item, index) => (
        <Badge
          key={index}
          className={cn(
            "px-2 py-1 rounded-xl flex items-center gap-1 hover:-translate-y-0.5 hover:underline transition-all cursor-pointer",
          )}
...
>(({ className, children }, ref) => {
  return (
    <CommandList
      ref={ref}
      className={cn(
        "p-2 flex flex-col gap-2 rounded-md scrollbar-thin scrollbar-track-transparent transition-colors scrollbar-thumb-muted-foreground dark:scrollbar-thumb-muted scrollbar-thumb-rounded-lg w-full absolute bg-background z-10 border border-muted top-0",
        className
      )}
    >
      {children}
...
>(({ className, value, children, ...props }, ref) => {
  ...
  return (
    <CommandItem
      ...
      className={cn(
        "rounded-md cursor-pointer p-2 tracking-wide transition-colors flex justify-between ",
        className,
    ...
    >
      {children}
      {isIncluded && <Check className="h-4 w-4 stroke-[3px] text-sky-800" />}
    </CommandItem>
  );
});
